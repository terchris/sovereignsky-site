...
/**
 * Split world.geo.json into individual country GeoJSON files.
 * 
 * Reads regions.json to determine which countries are needed,
 * extracts each country from world.geo.json,
 * and writes to static/data/countries/{ISO}.json
 * 
 * Usage: node scripts/split-world-geojson.js
 */
...
const WORLD_GEOJSON_PATH = path.join(STATIC_DATA_DIR, 'world.geo.json');
...
function main() {
  ...
  console.log('\nReading world.geo.json...');
  const world = readJson(WORLD_GEOJSON_PATH);
  
  if (!world.features || !Array.isArray(world.features)) {
    console.error('Error: world.geo.json does not have a features array');
    process.exit(1);
  }
  
  console.log(`Found ${world.features.length} features in world.geo.json`);
  ...
  if (missing.length > 0) {
    console.log(`\n⚠ Missing countries (not found in world.geo.json): ${missing.join(', ')}`);
  }
